...
1. We still support a potential privledged client
2. ProQuake:  fix for attack that crashes server
3. "There is already a test/rcon in progress" message (from JoeQuake I think, maybe ProQuake)
4. A could not open socket message.
5. A ProQuake reconnecting client fix, essentially this is commented out
			//NET_Close(s);
			//return NULL;
6. Another ProQuake fix:
	"Unexpected repsonse to Player Info request" isn't a Sys_Error, just Con_Print


Note: Quakespasm has another IN_Activate/IN_Deactivate in here.
  Not sure why those are all over the place in the source code???
...
static struct
{
	...
	byte			data[MAX_FITZQUAKE_DATAGRAM];
} packetBuffer;
...
//	Con_Printf ("Sending data to %s with length of %i (max: %i) with maxsize of %i\n", sock->address,  data->cursize, data->maxsize, sv.datagram.maxsize);
	...
	if (data->cursize <= MAX_FITZQUAKE_DATAGRAM)
	{
	...
	{
		dataLen= MAX_FITZQUAKE_DATAGRAM;
		eom = 0;
	}
...
static int SendMessageNext (qsocket_t *sock)
{
	...
	if (sock->sendMessageLength <= MAX_FITZQUAKE_DATAGRAM)
	{
...
int	Datagram_GetMessage (qsocket_t *sock)
{
	...
	while(1)
	{
		length = (unsigned int)sfunc.Read (sock->socket, (byte *)&packetBuffer,
			NET_FITZQUAKE_DATAGRAMSIZE, &readaddr); // Baker this is correct, we cap sending to 1024 if NetQuake but receiving to 1400
...
//#ifdef SUPPORTS_NETWORK_FIX // Baker change +
		...
		if (length > NET_FITZQUAKE_DATAGRAMSIZE)
		{
...
//#endif // Baker change +
		...
		if (flags & NETFLAG_ACK)
		{
			...
			sock->sendMessageLength -= MAX_FITZQUAKE_DATAGRAM;
			if (sock->sendMessageLength > 0)
			{
				memmove (sock->sendMessage, sock->sendMessage + MAX_FITZQUAKE_DATAGRAM, sock->sendMessageLength);
				sock->sendNext = true;
			}
			else
			{
...
#endif // Baker change -  (Note ... was Sys_Error = crash)
