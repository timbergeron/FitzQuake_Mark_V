...
again:
	...
	if (m_net_cursor == 0 /* && !ipxAvailable always true now*/)
		goto again;
...
// Range  (hi/low)
// Action
typedef void (*qboolfunc_t)(qboolean);
typedef enum { mdraw_none, mdraw_check, mdraw_slide, mdraw_vmode} mdraw_t;
...
void M_AlwaysRun_Toggle (qboolean action)
{
	if (action)
	{
...
void M_Pitch_Toggle (qboolean action)
{
	Cvar_SetValue ("m_pitch", -m_pitch.value);
}

const moptions_t menu_options_draw [] =
{
...
// slider: #increments, low, high
	...
	{opt_brightness_04, "            Brightness", mdraw_slide, &scr_brightness, 21, 1, 0.5}, 
	{opt_mousespeed_05, "           Mouse Speed", mdraw_slide, &sensitivity, 	11, 1, 11}, 
	{opt_cdvolume_06, 	"       CD Music Volume", mdraw_slide, &bgmvolume, 		11, 0, 3}, 
...
// mdraw_check: operation and value.
	...
	{opt_lookspring_11, "            Lookspring", mdraw_check, &lookspring, 		op_nonzero}, 
	{opt_lookstrafe_12, "            Lookstrafe", mdraw_check, &lookstrafe, 		op_nonzero },
...
void M_AdjustSliders (int options_cursor, int dir)
{
	...
	case mdraw_slide:
		{
			qboolean slider_positive = myopt->c > myopt->b;
...
void M_Options_Draw (void)
{
	...
	for (i = 0; i < num_menu_options_draw; i ++)
	{
		int row = 32 + i * 8;
		...
		case mdraw_slide:
			...
			break;
		case mdraw_vmode:
			M_Print (col_220, row, va("%ix%i", vid.screen.width, vid.screen.height) );
		default: 
			break;
		}
	}
...
void M_Options_Key (int k)
{
	...
	case K_ENTER:
		...
		case opt_videomode_13:
			...
			break;
		default:
			M_AdjustSliders (options_cursor, 1);
			break;
		}
		return;
...
void M_Quit_Draw (void) //johnfitz -- modified for new quit message
{
	...
	q_snprintf(msg1, sizeof(msg1), "%s version %1.2f", ENGINE_NAME, (float)FITZQUAKE_VERSION);
	q_snprintf(msg2, sizeof(msg2), "%s", "by John Fitzgibbons");
	q_snprintf(msg3, sizeof(msg3), "%s", "Press y to quit");
	...
	boxlen = q_max(strlen(msg1),q_max(strlen(msg2),strlen(msg3))) + 1;
	if (boxlen & 1) boxlen++;
