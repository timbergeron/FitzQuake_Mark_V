...
A. Fast get edict might need a tweak for 64-bit.

Versus Quakespasm: 

1. sv_novis, sv_bouncedownslopes, sv_cullentities support.
2. Rotating entities support (true rotation, not hipnotic).
...
cvar_t	sv_novis = {"sv_novis","0",CVAR_NOTIFY|CVAR_SERVERINFO};
cvar_t	sv_bouncedownslopes = {"sv_bouncedownslopes","0",CVAR_NOTIFY|CVAR_SERVERINFO};
cvar_t	sv_cullentities = {"sv_cullentities", "0",CVAR_NOTIFY|CVAR_SERVERINFO}; // OFF

cvar_t	sv_fishfix        = {"sv_fishfix",        "1", CVAR_NONE};
cvar_t	sv_imp12hack      = {"sv_imp12hack",      "1", CVAR_NONE};
...
void SV_PushRotate (edict_t *pusher, float movetime)
{
   ...
   if (!pusher->v.avelocity[0] && !pusher->v.avelocity[1] && !pusher->v.avelocity[2])
   {
	...
	//johnfitz -- dynamically allocate
	mark = Hunk_LowMark ();
	moved_edict		= Hunk_AllocName (sv.num_edicts * sizeof(edict_t *), "moved_ed");
	moved_from		= Hunk_AllocName (sv.num_edicts * sizeof(vec3_t),    "moved_fr");
	//johnfitz
...
//
#ifdef SUPPORTS_MH_NOCLIP_NOTRIGGER
	...
	if (ent->v.impulse == 12.0)
	{
		...
		if (((sv_imp12hack.value >= 2) || (sv_imp12hack.value && !pr_handles_imp12)) && 
