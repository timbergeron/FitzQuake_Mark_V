...
1.  Mark V prevents replacement textures in id1 from affecting models in a
	gamedir.  For example, someone has a replacement texture for a fiend in 
	the id1 gamedir.  But is running a mod where the fiend is reskinned in
    the gamedir.  Mark V won't load an id1 replacement texture for a non-id1
    model.
...
{
	return Image_LoadImage_Limited (name, width, height, NULL /* no media owner path */);
}
