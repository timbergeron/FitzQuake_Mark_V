...
1. Demo rewind
2. Color mapping everything.
3. Correct lighting on moved/rotated brush models
4. AVI capture
...
efrag_t			cl_efrags[MAX_FITZQUAKE_EFRAGS];
entity_t		cl_static_entities[MAX_FITZQUAKE_STATIC_ENTITIES];
lightstyle_t	cl_lightstyle[MAX_LIGHTSTYLES];
dlight_t		cl_dlights[MAX_FITZQUAKE_DLIGHTS];

entity_t		*cl_entities; //johnfitz -- was a static array, now on hunk
int				cl_max_edicts; //johnfitz -- only changes when new map loads

int				cl_numvisedicts;
entity_t		*cl_visedicts[MAX_FITZQUAKE_VISEDICTS];
...
// clear other arrays
	...
	case PROTOCOL_NETQUAKE:
		cl_max_edicts = MAX_SANE_EDICTS_8192; // Client-only and max_edicts is 0.  Set to 8192.  cl edicts are tiny.
		break;

	case PROTOCOL_FITZQUAKE:
	case PROTOCOL_FITZQUAKE_PLUS:
		cl_max_edicts = sv.active ? sv.max_edicts : MAX_SANE_EDICTS_8192; // Client-only and max_edicts is 0.  Set to 8192.  cl edicts are tiny.
		break;
	}

	cl_max_edicts = CLAMP (MIN_SANE_EDICTS_512, cl_max_edicts, MAX_SANE_EDICTS_8192);
...
//
	...
	for (i=0 ; i< MAX_FITZQUAKE_EFRAGS - 1 ; i++)
...
void CL_NextDemo (void)
{
	...
	q_snprintf (str, sizeof(str), "nextstartdemo %s\n", cls.demos[cls.demonum]);
	Cbuf_InsertText (str);
	cls.demonum++;
}
...
//		return;
	...
	time = fabs(cl.time - cl.oldtime); // forward oriented time even if demo rewinding
...
// rotate binary objects locally
		...
		if (cl_numvisedicts < MAX_FITZQUAKE_VISEDICTS)
		{
...
//johnfitz -- devstats
	...
	//beams
	for (i=0, b=cl_beams ; i< MAX_FITZQUAKE_BEAMS ; i++, b++)
...
//johnfitz
	...
	if (cls.demoplayback && cls.capturedemo /*cls.demonum == -1 && !cls.timedemo && !cls.titledemo*/)
	{
		...
		while (1)
		{
			...
			q_snprintf (tempstring, sizeof(tempstring), "Demo: %s (%3.1f%% elapsed: %4.1f secs) - Estimated Remaining %d:%02d (Capturing: %s)", cls.demoname, completed_amount * 100, cls.demo_hosttime_elapsed, (int)minutes, (int)seconds, movie_codec);

			Sys_WindowCaptionSet (tempstring);
			break;
		}
...
void CL_Init (void)
{
	...
	Cvar_RegisterVariable (&cl_titledemos_list);
	...
	Cmd_AddCommand ("cl_hints", CL_Hints_List_f);
}
