...
#if !defined(SUPPORTS_FAST_FIELD_LOOKUP) // Baker change (Obsoleted this)
/*
============
GetEdictFieldValue
============
...
=============
*/
static const char *PR_ValueString (etype_t type, eval_t *val)
{
...
=============
*/
static const char *PR_UglyValueString (etype_t type, eval_t *val)
{
	static char	line[256];
	ddef_t		*def;
	dfunction_t	*f;
...
dfunction_t *PR_FindNextFunction (int *start, const char *name, int flags)
{
	dfunction_t	*func;
	int			len, i;
	int			(*cmpfunc)(const char *, const char *, int);

	cmpfunc = (flags & PRFF_IGNORECASE ? Q_strncasecmp : Q_strncmp);
...
// byte swap the lumps
	...
	if (!pr_handles_imp12)
	{
		if (sv_imp12hack.value)
			Con_Warning ("Override progs.dat no impulse 12\n");
	}
