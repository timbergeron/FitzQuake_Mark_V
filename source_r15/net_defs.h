...
#define	NET_FITZQUAKE_DATAGRAMSIZE	(MAX_FITZQUAKE_DATAGRAM + NET_HEADERSIZE)
...
typedef struct qsocket_s
{
	...
	byte			sendMessage [NET_FITZQUAKE_MAXMESSAGE];

	unsigned int	receiveSequence;
	unsigned int	unreliableReceiveSequence;
	int				receiveMessageLength;
	byte			receiveMessage [NET_FITZQUAKE_MAXMESSAGE];

	struct qsockaddr	addr;
	char				address[NET_NAMELEN];

	byte            proquake_connection; // ProQuake full NAT connect (1/6)
} qsocket_t;
