...
1. Support for multiple poly draw types.
2. cl.ctime (reverse sky scroll if demo rewind is on)
...
void Sky_LoadTexture (texture_t *mt)
{
	...
	if (mt->width != 256 || mt->height != 128)
		Host_Error ("Sky texture %s must be 256 x 128 but is %i by %i \n", mt->name, mt->width, mt->height);
...
#if 1 //also accept non-standard keys (Fitz 0.85)

	else if ((valuestring = COM_CL_Worldspawn_Value_For_Key ("skyname"))) //half-life
		Sky_LoadSkyBox(valuestring);
	else if ((valuestring = COM_CL_Worldspawn_Value_For_Key ("q1sky"))) //quake lives
		Sky_LoadSkyBox(valuestring);
#endif	
...
void Sky_DrawFaceQuad (glpoly_t *p)
{
	...
	if (renderer.gl_mtexable && r_skyalpha.value >= 1.0)
	{
...
void Sky_DrawSky (void)
{
	...
	glDisable (GL_TEXTURE_2D);
	if (Fog_GetDensity() > 0)
		glColor3fv (Fog_GetColor(NULL, NULL));
	else
