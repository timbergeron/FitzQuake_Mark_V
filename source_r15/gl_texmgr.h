...
1. Mark V supports WAD3, alpha textures
2. Mark V supports colormapped skins of non-players including dead bodies
...
//managed by texture manager
	...
	qmodel_t			*owner;
// Baker
	unsigned int		bsp_texnum; // for bsp
#if 0
	char				paste_texture_name[MAX_QPATH]; //relative filepath to data source, or "" if source is in memory
...
extern unsigned int d_8to24table_fbright_alpha[256]; // fbright pal but 255 has no alpha
...
gltexture_t *TexMgr_LoadImage_SetPal (qmodel_t *owner, int bsp_texnum, const char *name, int width, int height, enum srcformat format,
								   byte *data, byte *palette_data, const char *source_file, unsigned source_offset, src_offset_t source_palette_offset, unsigned flags);
...
#define MAX_COLORMAP_SKINS 1024
extern gltexture_t *playertextures[MAX_COLORMAP_SKINS];

qboolean TexMgr_Clipboard_Set (gltexture_t *glt);
const char* TexMgr_TextureModes_ListExport (void); // Baker

#define PALETTE_SIZE_768 768
