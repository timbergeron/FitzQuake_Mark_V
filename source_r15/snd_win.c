...
// only release primary buffer if it's not also the mixing buffer we just released
	...
	if (pDS)
	{
		IDirectSound_SetCooperativeLevel (pDS, wplat.mainwindow, DSSCL_NORMAL);
...
sndinitstat SNDDMA_InitDirect (void)
{
	...
	if (DS_OK != IDirectSound_SetCooperativeLevel (pDS, wplat.mainwindow, DSSCL_EXCLUSIVE))
	{
...
// sound hardware format
	...
	{
		if (DS_OK != IDirectSound_SetCooperativeLevel (pDS, wplat.mainwindow, DSSCL_WRITEPRIMARY))
		{
			Con_SafePrintf ("Set coop level failed\n");
			FreeSound ();
			return SIS_FAILURE;
