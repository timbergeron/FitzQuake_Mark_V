...
1. Elimination of WinQuake relics
2. Support for demo rewind
...
entity_t	cl_temp_entities[MAX_FITZQUAKE_TEMP_ENTITIES];
beam_t		cl_beams[MAX_FITZQUAKE_BEAMS];
...
static entity_t *CL_NewTempEntity (void)
{
	...
	if (cl_numvisedicts == MAX_FITZQUAKE_VISEDICTS)
		return NULL;
	if (num_temp_entities == MAX_FITZQUAKE_TEMP_ENTITIES)
		return NULL;
...
// ent->colormap = vid.colormap; Baker: WinQuake only ... our default is 0 so no need for GL colored bodies
	return ent;
}
