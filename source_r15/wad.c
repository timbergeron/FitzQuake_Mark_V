...
1. More informative "gfx.wad not found" message
	since this is a common problem for a new user who accidentally 
	places the engine in the wrong place.
*/
...
void W_LoadWadFile (void) //johnfitz -- filename is now hard-coded for honesty
{
	...
	if (!wad_base)
	{
			Sys_MessageBox ("Quake Not Detected", "Your Quake folder should contain a "
							"folder named id1 with pak0.pak and pak1.pak.\n\nOpening folder ...");

			Sys_OpenFolder_Highlight_Binary();
			...
			Sys_Error ("W_LoadWadFile: couldn't load %s\n\n"
						...
						"Is FitzQuake.exe in the proper folder?", filename);
	}
