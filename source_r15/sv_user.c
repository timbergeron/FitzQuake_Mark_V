...
1. We are keeping priviledged client support (that can execute any command)
2. "freezeall" command. (instead of sv_freezenonclients cvar ... what purpose
   does that have as a cvar?  Mark V has it as a standard "command" that
   any client can do just like noclip or pause).
...
//				Sys_Printf ("clc_nop\n");
			...
			case clc_stringcmd:
				...
				if (Q_strncasecmp(s, "status", 6) == 0)
					...
					ret = 1;
				else if (Q_strncasecmp(s, "freezeall", 9) == 0)
					ret = 1;
