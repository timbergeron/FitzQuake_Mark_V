...
1. Scoreboard pings.
2. Lack of Quakespasm's altered scoreboard.  May implement something
   showing skill.
3. Title demo support (No sbar draw, no crosshair, fov fixed, etc.)
...
cvar_t	scoreboard_pings = {"scoreboard_pings", "1", CVAR_NONE};
...
void Sbar_Init (void)
{
	...
	Cvar_RegisterVariable (&scoreboard_pings);
	Sbar_LoadPics ();
}
...
{
	int		minutes = cl.ctime / 60;
	int		seconds = (int)cl.ctime % 60;
	char	str[256];
	q_snprintf (str, sizeof(str), "%i:%02i", minutes, seconds);
...
void Sbar_SoloScoreboard (void)
{
	...
	if (cl.gametype != GAME_DEATHMATCH)
	{
		...
		{
			int _cl_skill_int = sv.active ? (int)(skill.value  + 0.5) : (cl.skillhint - 1);
...
// draw the text
	numscores = q_min (scoreboardlines, 2);  // Baker:  Was 4 -- now 2 to make sure the clock draws!
...
// PGM 03/02/97 - fixed so color swatch only appears in CTF modes
	if (rogue && (cl.maxclients != 1) && (teamplay.value>3) && (teamplay.value<7))
	{
		...
		if (cl.gametype == GAME_DEATHMATCH)
			...
			xofs = ((vid.screen.width - 320)>>1) + 113;
...
void Sbar_Draw (void)
{
	...
	if (scr_con_current == vid.screen.height)
	...
	if (sb_updates >= vid.numpages && !gl_clear.value && scr_sbaralpha.value >= 1 && !renderer.isIntelVideo && !vid.fullsbardraw) //johnfitz -- gl_clear, scr_sbaralpha, intel workarounds from baker
		return;
   ...
   // armor
	...
	if (cl.gametype == GAME_DEATHMATCH || sb_showscores || sbar_clock.value)
		Sbar_DrawClock ();
...
void Sbar_DeathmatchOverlay (void)
{
	...
	if (scoreboard_pings.value && cl.last_ping_time < cl.time - 5)
	{
...
// draw the text
	...
	//  draw ping
		if (s->ping && scoreboard_pings.value)
		{
			ping = 1;
	...
	// draw name
		...
		M_PrintWhite (x+64, y, s->name); //johnfitz -- was Draw_String, changed for stretched overlays	
...
// draw the text
    ...
    if (i == scoreboardlines) // we're not there
            ...
            i = i - numlines/2;
    CLAMP (0, i, scoreboardlines - numlines);
	if (i < 0) i = 0; // "scoreboardlines-numlines" can result in a negative number, so we need this.  Baker  An example is a 4 player deathmatch (4 participants - 6 lines = -2!!!)
