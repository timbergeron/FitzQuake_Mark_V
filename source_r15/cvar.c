...
1. Quakespasm has registering a callback and variable registration as
   2 functions.  We use a single function.
2. "dec" command that decrements a cvar
*/
...
#ifdef ADD_DEC_CVAR // Baker change
...
void Cvar_SetValueQuick (cvar_t *var, const float value)
{
	...
	{
		q_snprintf (val, sizeof(val), "%g", value); // Baker: Why isn't Quakespasm using %g instead of %f ?
#if 0
...
============
*/
static void Cvar_SetCallback (cvar_t *var, cvarcallback_t func)
{
	var->callback = func;
	if (func)
...
{
	Cvar_SetCallback (variable, func);
