...
1. ProQuake bestweapon support (DarkPlaces and virtually every other Quake
   engine support this).
2. Leaning against +speed key being a +slow key if always run isn't on, this
   hardcoded idea may alter the intended behavior of some existing mods that
   do not have cl_forwardspeed 200 = normal ... for starters.
...
cvar_t	cl_forwardspeed = {"cl_forwardspeed","400", CVAR_ARCHIVE}; // Baker: Always run default, was 200
cvar_t	cl_backspeed = {"cl_backspeed","400", CVAR_ARCHIVE}; // Baker: Always run default, was 200
...
#if 1
		if (cl.protocol == PROTOCOL_NETQUAKE  && (cls.demoplayback || NET_QSocketIsProQuakeServer(cls.netcon) == false) )
#else
